...
static int32_t netdisk_handler(int first_sockfd, zlog_category_t *zlog_handle);
...
int32_t netdisk_connect(zlog_category_t *zlog_handle)
{
	...
	int first_listenfd;
	int first_connfd;
	static int num = MAX_EVENTS;
	struct sockaddr_in servaddr, cliaddr;
	socklen_t len = sizeof(cliaddr);
	struct epoll_event ev, events[MAX_EVENTS];
	
	printf("Starting create first connection.\n");
	...
	printf("Server starts to listen the port.\n");
	...
	fprintf(stdout, "Listening the port...\n");
	...
	while (1)
	{
		...
		nready = epoll_wait(epollfd, events, num, -1);
		if (nready == -1)
		{
			if (errno == EINTR)
			{
		...
		for (i = 0; i < nready; i++)
		{
			if (events[i].data.fd == first_listenfd)
			{
				...
				printf("The server connected,waiting for data connection\n");

				flag = netdisk_nonblock(first_connfd);
				if (flag == -1)
				{
					zlog_error(zlog_handle, "netdisk_nonbloak() error");
					return -1;
				}
			...
			else if (events[i].events & EPOLLIN)
			{
				...
				netdisk_handler(first_connfd, zlog_handle);
				close(first_connfd);
			}
		}
	}
...
static int32_t netdisk_handler(int first_connfd, zlog_category_t *zlog_handle)
{
	int ret;

	ret = netdisk_cmd_recv(first_connfd, zlog_handle);
	if (ret == -1)
	{
		zlog_error(zlog_handle, "netdisk_cmd_recv() error");
		return -1;
	}
