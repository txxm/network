...
static void netdisk_reg_handler(int first_sockfd, zlog_category_t *zlog_handle);
...
int32_t netdisk_reg_connect(zlog_category_t *zlog_handle)
{
	...
	printf("#####Starting register connection.\n");
	...
	fprintf(stdout, "#####Listening register port...\n");
	...
	while (1)
	{
		printf("#####Waiting for client's register request\n");
		...
		for (i = 0; i < nready; i++)
		{
			...
			else if (events[i].events & EPOLLIN)
			{
				...
				netdisk_reg_handler(first_connfd, zlog_handle);
				close(first_connfd);
			}
		}
	}
...
static void netdisk_reg_handler(int first_connfd,
									zlog_category_t *zlog_handle)
{
	int ret;

	ret = netdisk_reg_recv(first_connfd, zlog_handle);
	if (ret == -1)
	{
		zlog_error(zlog_handle, "netdisk_reg_recv() error");
		return ;
	}
