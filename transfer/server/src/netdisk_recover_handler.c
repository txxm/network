...
sk0:nrecv = recv(first_connfd, filename, sizeof(filename), MSG_DONTWAIT);
	...
	sprintf(oldname, "../user/%s/recycle/%s", filename, cmd_info_ptr->third);

	memset(newname, 0, FILENAME_LEN);
	sprintf(newname, "../user/%s/recover/%s", filename, cmd_info_ptr->third);
...
sk2:	nwrite = write(fd2, buf, nread);
	...
	ret = fchmod(fd2, RDWR_DEF);
	if (ret == -1)
	{
		zlog_error(zlog_handle, "fchmod() error");
		return -1;
	}
